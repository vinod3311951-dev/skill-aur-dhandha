...
function start(id=state.currentStage){try{state.currentStage=Math.max(1,Math.min(CORE_STAGE_COUNT,id));state.firstPlay=false;save();const c=STAGES[state.currentStage-1],generated=generateValidBoard(c,3);engine=new GameEngine({board:generated.board,config:{...c,seed:generated.config.seed}});pulseMusic.setWorld(c.world);overlay.hidden=true;pauseOverlay.hidden=true;paused=false;renderDirty=true;show("play");hud();applySettings();st.textContent="Aim anywhere above the launcher, then release."}catch(err){showError(err)}}
