...
function applySettings(){document.body.classList.toggle("reduced-motion",state.settings.reducedMotion);[sting,launchSfx,popSfx,dropSfx,comboSfx,bloomSfx,failSfx].forEach(a=>a.muted=!state.settings.sfx);if(state.settings.music&&screens.play&&!screens.play.hidden&&!paused){pulseMusic.start().catch(()=>{})}else pulseMusic.stop()}
...
function bubble(x,y,R,c){const base=COLOR_HEX[c],light=mix(base,[255,255,255],.52),mid=mix(base,[255,255,255],.12),dark=mix(base,[8,14,34],.42);ctx.save();ctx.shadowColor=alpha(base,.42);ctx.shadowBlur=Math.max(3,R*.38);ctx.shadowOffsetY=Math.max(1,R*.08);const fill=ctx.createRadialGradient(x-R*.38,y-R*.42,Math.max(1,R*.06),x+R*.10,y+R*.14,R*1.16);fill.addColorStop(0,"rgba(255,255,255,.98)");fill.addColorStop(.13,light);fill.addColorStop(.42,mid);fill.addColorStop(.78,base);fill.addColorStop(1,dark);ctx.beginPath();ctx.arc(x,y,R-1,0,Math.PI*2);ctx.fillStyle=fill;ctx.fill();ctx.shadowColor="transparent";ctx.strokeStyle="rgba(255,255,255,.72)";ctx.lineWidth=Math.max(1.5,R*.085);ctx.stroke();ctx.beginPath();ctx.ellipse(x-R*.29,y-R*.34,R*.29,R*.15,-.5,0,Math.PI*2);ctx.fillStyle="rgba(255,255,255,.42)";ctx.fill();ctx.fillStyle="#fff";ctx.font=`800 ${Math.max(13,R*.78)}px system-ui`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(SYMBOLS[c],x,y+.5);ctx.restore()}
