...
canvas.addEventListener("pointerup",e=>{if(aim&&engine&&g&&!paused){const fired=engine.fire(g.launcher,pos(e));if(fired){sfx(launchSfx);haptic(8);analytics.track("cd_shot",{level:engine.config.id,seed:engine.config.seed,objectiveType:engine.objectiveState.type,shotCount:engine.shotNumber+1,activeColorCount:new Set(engine.board.values()).size})}aim=null}})
