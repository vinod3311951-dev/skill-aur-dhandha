...
// FIX: pulseBloom now clears itself after 1100ms. Previously it added .active and never removed it,
// which caused #bloom-layer to stay on screen permanently after the first match (the stuck purple board).
function pulseBloom(){if(reducedMotionActive())return;bloom.classList.remove("active");void bloom.offsetWidth;bloom.classList.add("active");clearTimeout(pulseBloom.cleanupTimer);pulseBloom.cleanupTimer=setTimeout(()=>{bloom.classList.remove("active")},1100)}
...
canvas.addEventListener("pointerdown",e=>{cascadeRestore.classList.remove("active");bloom.classList.remove("active");if(engine&&!paused&&!engine.projectile&&engine.status==="playing"){canvas.setPointerCapture?.(e.pointerId);aim=pos(e);renderDirty=true}})
