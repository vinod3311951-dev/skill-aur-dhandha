...
test("PWA repair uses a versioned complete core cache and deferred update activation",()=>{const sw=fs.readFileSync(new URL("../sw.js",import.meta.url),"utf8");assert.match(sw,/color-dominion-v16/);assert.match(sw,/src\/integrations\.js/);assert.match(sw,/CD-SFX-007-combo-rise-v1\.wav/);assert.doesNotMatch(sw,/install"[\s\S]{0,240}skipWaiting/);assert.match(sw,/SKIP_WAITING/)});
